...
library.add(faCheck, faTrash, faPencil)
                    ...
                    {this.props.main.todos.map(item => {
                        return (
                            <div className={this.props.main.active === item.id ? "todo-active todo" : "todo"} key={item.id}>
                                ...
                                {this.props.main.todos.length > 1 ? <button className="todo-delete" onClick={() => this.handleDelete(item)}><FontAwesomeIcon icon={faTrash} /></button> : <button className="todo-delete" disabled onClick={() => this.handleDelete(item)}><FontAwesomeIcon icon={faTrash} /></button>}
                            </div>
                        )
                    })}
                </div>
